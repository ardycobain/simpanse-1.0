...
import { Item, Loan, ViewType, ItemStatus, UserRole, SystemConfig, LoanStatus } from './types';
...
const App: React.FC = () => {
  ...
  }, [items, loans, sessionEmail, sessionNIK, sessionName, systemConfig]);
  ...
  const handleActionWithReason = useCallback((loanId: string, status: LoanStatus) => {
    ...
    const statusLabels: Record<string, string> = {
      'Approved': 'Disetujui',
      'Rejected': 'Ditolak',
      'ReviewRequired': 'Ditangguhkan'
    };
    
    setNotification({ message: `Status diperbarui: ${statusLabels[status] || status}.`, type: 'success' });
  }, [loans]);
  ...
  const getStatusActionMessage = (status: LoanStatus, reason?: string) => {
    switch(status) {
      ...
      case 'ReviewRequired': alert("Mohon maaf, aset yang anda pinjam masih membutuhkan review lebih lanjut. Mohon menunggu atau hubungi nomor kontak UPT untuk informasi lebih lanjut."); break;
      case 'Rejected': alert(`Mohon maaf, aset yang anda pinjam ditolak.\n\nAlasan: ${reason || 'Kebijakan internal'}\n\nHubungi nomor kontak UPT untuk informasi lebih lanjut.`); break;
      case 'Verified': alert("Data Anda sudah terverifikasi oleh Admin. Saat ini sedang menunggu persetujuan akhir dari Verifikator."); break;
      case 'Pending': alert("Permintaan Anda sedang dalam antrian pemeriksaan Admin. Mohon bersabar."); break;
  ...
  return (
    <div className="min-h-screen bg-white flex">
      ...
      <main className="flex-1 ml-64 min-h-screen flex flex-col bg-slate-50/50">
        <div className="flex-1">
          ...
          <div className="max-w-[95%] mx-auto p-8">
                ...
                <div className="border-b-4 border-yellow-400 pb-6 flex justify-between items-end">
                  <div>
                    <h2 className="text-3xl font-black text-blue-600 tracking-tight">
                      {isStaff ? 'Laporan Aktivitas Peminjaman' : 'Status Pengajuan Saya'}
                    </h2>
                    <p className="text-slate-500 text-sm font-medium mt-1 uppercase tracking-widest">
                      {isStaff ? 'Panel Kontrol Manajemen Aset UPT' : 'Klik pada status untuk informasi detail'}
                    </p>
                  </div>
                  {!isStaff && sessionName && (
            ...
            {currentView === 'super-admin-users' && <SuperAdminDashboard />}
        ...
        <footer className="bg-white border-t-8 border-yellow-400 p-16 mt-auto">
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center space-y-12 md:space-y-0">
            ...
            {/* Tengah: Sosial Media & Website (DIPOSISIKAN DI TENGAH) */}
            <div className="flex-1 flex flex-col items-center justify-center space-y-6">
              ...
              <div className="flex items-center space-x-4">
                ...
                {systemConfig.socialFB && (
                  <a href={systemConfig.socialFB} target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-slate-100 text-blue-600 rounded-2xl flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all shadow-sm hover:shadow-lg" title="Facebook">
                ...
                {systemConfig.socialYT && (
                  <a href={systemConfig.socialYT} target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-slate-100 text-red-600 rounded-2xl flex items-center justify-center hover:bg-red-600 hover:text-white transition-all shadow-sm hover:shadow-lg" title="YouTube">
      ...
      {showLogin && <LoginGate onLogin={handleLogin} onCancel={() => setShowLogin(false)} />}
